	...
	:m_Width(width), m_Height(height)
{
	...
	BindFramebufferTexture2D(m_GNormal);
	BindFramebufferTexture2D(m_GAlbedoSpec);
...
void GBuffer::BindFramebufferTexture2D(unsigned int& rendererID)
{
	...
	GLCall(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, rendererID, 0));
}
